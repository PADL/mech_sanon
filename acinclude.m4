...
----------------------------------------------------------------------
	...
	AC_CHECK_LIB(gssapi_krb5, GSS_C_INQ_NEGOEX_KEY, [AC_DEFINE_UNQUOTED([HAVE_GSS_C_INQ_NEGOEX_KEY], 1, [Define if GSS-API library supports NegoEx])], [], "$KRB5_LIBS")
	AC_CHECK_LIB(gssapi_krb5, GSS_C_MA_NEGOEX_AND_SPNEGO, [AC_DEFINE_UNQUOTED([HAVE_GSS_C_MA_NEGOEX_AND_SPNEGO], 1, [Define if GSS-API library supports exposing mechanism through both NegoEx and SPNEGO]) negoex=yes], [negoex=no], "$KRB5_LIBS")
	AM_CONDITIONAL(NEGOEX, test "x$negoex" != "xno")
